...
import { type NavEntry, readNavCache, writeNavCache, parseNavDate } from "@/features/charts/fundNavUtils";
...
const FundNavChart: React.FC<FundNavChartProps> = ({ fund, onClose }) => {
  ...
  const [tooltip, setTooltip] = useState({ visible: false, x: 0, y: 0, date: "", nav: 0 });
  const [range, setRange] = useState<"1Y" | "3Y" | "5Y">("1Y");

  useEffect(() => {
    let cancelled = false;
    setLoading(true); setError(false); setHistory([]);
    ...
    fetch(`https://api.mfapi.in/mf/${fund.schemeCode}`)
      ...
      .then((j) => {
        if (cancelled) return;
        if (j.status !== "SUCCESS" || !j.data?.length) { setError(true); setLoading(false); return; }
        ...
        setHistory(entries); setLoading(false);
      })
  ...
  const data = React.useMemo(() => {
    ...
    cutoff.setFullYear(cutoff.getFullYear() - (range === "1Y" ? 1 : range === "3Y" ? 3 : 5));
  ...
  const changePct  = data.length ? ((currentNav - firstNav) / firstNav) * 100 : 0;
  ...
  useEffect(() => {
    ...
    data.forEach((pt, i) => { if (i > 0) areaD += ` L ${x(i)} ${y(pt.nav)}`; });
    areaD += ` L ${x(data.length - 1)} ${P + cH} L ${P} ${P + cH} Z`;
    const area = document.createElementNS("http://www.w3.org/2000/svg", "path");
    area.setAttribute("d", areaD); area.setAttribute("fill", `url(#gnav-${fund.schemeCode})`);
    svg.appendChild(area);

    let lineD = `M ${x(0)} ${y(data[0].nav)}`;
    data.forEach((pt, i) => { if (i > 0) lineD += ` L ${x(i)} ${y(pt.nav)}`; });
    ...
    sampled.forEach((pt) => {
      const px = x(data.indexOf(pt)), py = y(pt.nav), hw = cW / sampled.length;
      ...
      hit.addEventListener("mouseenter", () => {
        ...
        dot.setAttribute("id", "nav-hover-dot"); svg.appendChild(dot);
        ...
        vl.setAttribute("stroke-dasharray", "3,3"); vl.setAttribute("id", "nav-hover-line");
        svg.appendChild(vl);
  ...
  return (
    <div style={{ background: "hsl(var(--card) / 0.6)", backdropFilter: "blur(8px)", border: "1px solid hsl(var(--border))", borderRadius: "10px", overflow: "hidden", transition: "background 0.15s ease" }}>
      {/* Header */}
      <div style={{ padding: "16px 20px", borderBottom: "1px solid hsl(var(--border))", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
            <span style={{ fontSize: "13px", fontWeight: 500, letterSpacing: "-0.01em", color: "hsl(var(--foreground))" }}>{fund.name}</span>
            {isPos ? <TrendingUp size={13} style={{ color: "#22c55e", flexShrink: 0 }} /> : <TrendingDown size={13} style={{ color: "#ef4444", flexShrink: 0 }} />}
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: "8px" }}>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "15px", fontWeight: 500, letterSpacing: "-0.01em", color: "hsl(var(--foreground))" }}>₹{currentNav.toFixed(2)}</span>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", color: isPos ? "#22c55e" : "#ef4444" }}>{isPos ? "+" : ""}{changePct.toFixed(2)}% ({range})</span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <div style={{ display: "flex", borderRadius: "6px", border: "1px solid hsl(var(--border))", overflow: "hidden" }}>
            {(["1Y", "3Y", "5Y"] as const).map((r) => (
              <button key={r} onClick={() => setRange(r)} style={{ padding: "4px 10px", fontSize: "11px", fontWeight: range === r ? 500 : 400, background: range === r ? "hsl(var(--foreground))" : "transparent", color: range === r ? "hsl(var(--background))" : "hsl(var(--muted-foreground))", border: "none", cursor: "pointer", transition: "background 0.12s ease, color 0.12s ease" }}>{r}</button>
            ))}
          </div>
          <button onClick={onClose} style={{ width: "28px", height: "28px", borderRadius: "6px", background: "transparent", border: "1px solid hsl(var(--border))", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "hsl(var(--muted-foreground))", transition: "background 0.12s ease" }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "hsl(var(--secondary) / 0.7)"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "transparent"; }}>
            <X size={13} />
          </button>
        </div>
      </div>
      ...
      <div style={{ position: "relative", background: "transparent" }}>
        {loading && (
          <div style={{ height: "180px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "8px" }}>
            <Loader2 size={16} style={{ color: "hsl(var(--muted-foreground))", animation: "spin 0.8s linear infinite" }} />
            <span style={{ fontSize: "12px", color: "hsl(var(--muted-foreground))", fontWeight: 300 }}>Loading NAV history…</span>
          </div>
        )}
        {error && !loading && (
          <div style={{ height: "180px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", color: "hsl(var(--muted-foreground))" }}>
            Failed to load chart data.
          </div>
        )}
        {!loading && !error && (
          <>
            <svg ref={svgRef} style={{ width: "100%", height: "180px", display: "block" }} viewBox="0 0 400 160" preserveAspectRatio="xMidYMid meet" />
            {tooltip.visible && (
              <div style={{ position: "absolute", left: `${tooltip.x}px`, top: `${Math.max(8, tooltip.y - 52)}px`, transform: "translateX(-50%)", background: "hsl(var(--popover) / 0.9)", backdropFilter: "blur(8px)", border: "1px solid hsl(var(--border))", borderRadius: "7px", padding: "6px 10px", pointerEvents: "none", zIndex: 10, boxShadow: "0 4px 16px hsl(var(--foreground) / 0.08)", whiteSpace: "nowrap" }}>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", color: "hsl(var(--muted-foreground))", marginBottom: "2px" }}>{tooltip.date}</div>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "13px", fontWeight: 500, color: "hsl(var(--foreground))" }}>₹{tooltip.nav.toFixed(2)}</div>
              </div>
            )}
          </>
        )}
      ...
      {!loading && !error && data.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", borderTop: "1px solid hsl(var(--border))" }}>
          {[
            ...
            <div key={stat.label} style={{ padding: "12px 16px", textAlign: "center", borderRight: i < 2 ? "1px solid hsl(var(--border))" : "none" }}>
              <div style={{ fontSize: "10px", fontWeight: 500, letterSpacing: "0.05em", textTransform: "uppercase", color: "hsl(var(--muted-foreground))", marginBottom: "4px" }}>{stat.label}</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "12px", fontWeight: 500, color: stat.color }}>{stat.value}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Footer */}
      <div style={{ padding: "10px 16px", borderTop: "1px solid hsl(var(--border))", background: "hsl(var(--secondary) / 0.5)", display: "flex", alignItems: "center", gap: "10px" }}>
        ...
        <span style={{ fontSize: "11px", color: "hsl(var(--muted-foreground))", fontWeight: 300, marginLeft: "auto" }}>
          5Y CAGR: <strong style={{ color: "#22c55e", fontFamily: "'JetBrains Mono', monospace" }}>{fund.cagr5y.toFixed(1)}%</strong>
        </span>
      </div>
    </div>
  );
